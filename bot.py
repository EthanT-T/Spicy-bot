...
def get_ticket_overwrites(guild, user):
    """Génère les permissions de base pour un ticket (User + Staff + Manager)."""
    overwrites = {
...
class ModalRecrutement(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Recrutements vont dans ID_CATEGORIE_TICKETS
        ...
        if discord.utils.get(guild.text_channels, name=nom_salon):
            ...
            return await interaction.followup.send(f"❌ Tu as déjà un ticket de candidature ouvert : {existant.mention}", ephemeral=True)
...
class ModalSupport(discord.ui.Modal, title="Ouvrir un ticket support"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        guild = interaction.guild
        # Support va dans ID_CATEGORIE_SUPPORT
    ...
    async def btn_claim(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                if not staff: return await interaction.response.send_message("❌ Ton compte Discord n'est pas lié. Impossible de t'assigner le ticket.", ephemeral=True)
                ...
                if ticket['status'] in ['Pris', 'resolu', 'Résolu']: return await interaction.response.send_message("⚠️ Ce ticket est déjà en cours de traitement ou résolu.", ephemeral=True)
    ...
    async def btn_resolve(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                staff = cursor.fetchone()
                if not staff: return await interaction.response.send_message("❌ Ton compte n'est pas lié.", ephemeral=True)
                ...
                ticket = cursor.fetchone()
                if not ticket: return await interaction.response.send_message("❌ Ticket introuvable.", ephemeral=True)
                if ticket['status'] in ['resolu', 'Résolu']: return await interaction.response.send_message("⚠️ Ce ticket est déjà clos.", ephemeral=True)
...
class LierCompteModal(discord.ui.Modal, title="Liaison de compte Steam"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            print(f"Erreur liaison compte: {e}")
            await interaction.response.send_message("❌ Une erreur technique est survenue.", ephemeral=True)
        finally:
            if 'conn' in locals() and conn.open: conn.close()
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            for ev in new_events:
                # Logique de création d'événements
                pass
    except Exception as e:
        pass
    finally:

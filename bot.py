...
1. LE SITE WEB (Panel) : L'adresse est https://spicy-anomaly.alwaysdata.net. C'est un panel ultra-moderne avec des animations 3D (pas de thèmes RP ennuyeux). Les joueurs y voient le classement mondial (ratio K/D), leurs stats, et peuvent gérer leurs demandes de déban/unmute.
2. LA PERSONNALISATION DE PROFIL : Sur le site, les joueurs peuvent modifier le fond d'écran de leur carte de profil (Lien MP4/Image) au Niveau 5, ajouter une musique (Lien MP3) au Niveau 10, et un Badge exclusif au Niveau 15. Le Staff a tout débloqué d'office.
3. LE DISCORD & LE BOT : Pour avoir ses stats, un joueur DOIT lier son compte en cliquant sur le bouton de liaison et en entrant son SteamID64. S'il s'est trompé, il peut utiliser la commande /delier. Les commandes /stats, /profil et /casier permettent de voir sa progression.
4. L'XP ET LES NIVEAUX : On gagne de l'XP en tuant ou en s'échappant en jeu. Sur Discord, les rôles de niveaux (Niveau 1, Niveau 5, Niveau 10...) s'actualisent tous les 5 niveaux automatiquement.
5. LE MVP DE LA SEMAINE : Tous les dimanches à 20h00 UTC, le bot regarde la colonne 'weekly_xp'. Le joueur ayant farmé le plus d'XP dans la semaine reçoit le rôle Discord "🌟 MVP de la Semaine", une annonce publique est faite, et il gagne la couronne 🏆 sur le site web. L'XP de la semaine est ensuite remise à zéro.
6. LES PLUGINS EN JEU : 
   - Actifs : ProjectMoeR (gestion des spawns/mécaniques), AudioPlayer (musique), DoorSystem, StartSystem (gestion map et début de round), Plugins de Tickets, de Stats et de Niveaux.
   - Inactif : OmegaWire (il est installé mais désactivé).
7. LE STAFF : Divisé en plusieurs rôles. Le rôle "Spicy Team" (modérateurs/animateurs) gère les tickets de support en jeu et sur Discord. Le rôle "Manager" (et fondateurs) a tous les droits, ce sont les seuls à pouvoir lire les tickets de candidatures (recrutement). Un système de warn (/warn) empêche un membre du staff de warn un autre membre du staff (seuls les fondateurs peuvent le faire).
8. LE RÈGLEMENT RAPIDE : Pas d'alliance Inter-Factions (SCP/Humains ou MTF/Chaos). Pas de Teamkill (TK) ni de Spawnkill (sauf dans la fondation). Soundboards autorisés mais sans abus. Interdiction de se déconnecter en combat (Combat Log). Interdiction de bloquer SCP-914. En zone d'administration (Jail), on ne se déconnecte pas et on reste poli.
9. LE SYSTÈME DE QUÊTES : Les joueurs reçoivent des missions quotidiennes (tuer des SCP, s'échapper, etc.). Ils peuvent voir leurs objectifs avec la commande /quetes. C'est un excellent moyen de gagner de l'XP bonus rapidement !

Si on te pose une question sur un joueur précis ou un bannissement, dis d'ouvrir un ticket ou d'aller sur l'onglet 'Mes Sanctions' du panel web. Tu ne peux pas débannir toi-même.
"""
...
def init_db():
    try:
        ...
        with conn.cursor() as cursor:
            ...
            """)
            # Table des quêtes pour les joueurs
            cursor.execute("""
